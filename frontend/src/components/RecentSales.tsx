...
export function RecentSalesBlock() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["recent-sales", range.from, range.to, page],
    queryFn: () => getRecentSales(range, page * PAGE_SIZE, PAGE_SIZE),
  });
  ...
  return (
    <div className="rounded-2xl border border-[#232a44] bg-[#151a2d] p-5">
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        ...
        <button
          onClick={() =>
            downloadCsv(`recent-sales-${range.from}-${range.to}.csv`, items)
          }
